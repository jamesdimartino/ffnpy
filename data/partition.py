...
flags.DEFINE_list('lom_radius', [24,24,24],
...
def _query_summed_volume(svt, diam):
    ...
    """
    assert diam[0] < svt.shape[0], "diam[0] exceeds svt.shape[0]"
    assert diam[1] < svt.shape[1], "diam[1] exceeds svt.shape[1]"
    assert diam[2] < svt.shape[2], "diam[2] exceeds svt.shape[2]"
...
def compute_partitions(seg_array, thresholds, lom_radius, min_size=10000):
    ...
    #print(f"{lom_diam_zyx}")
    valid_sel = [_sel(x) for x in lom_radius_zyx]
    print(f"validsel: {valid_sel}")
    valid_sel_tuple = tuple(valid_sel)  # Ensure valid_sel is a tuple

    # Debugging information
    print("validsel:", valid_sel)
    print("validsel_tuple:", valid_sel_tuple)
    print("Type of validsel_tuple:", type(valid_sel_tuple))
    output = np.zeros(seg_array[valid_sel_tuple].shape, dtype=np.uint8)
    corner = lom_radius
    ...
    for l in labels:
        ...
        object_mask = object_mask[valid_sel_tuple] 
