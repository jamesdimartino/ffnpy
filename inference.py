...
def make_canvas(corner, subvol_size, model, image_path):
    ...
    subvol_size = [int(s) for s in subvol_size]
    end_points = [c + s for c, s in zip(corner, subvol_size)]
    
    # Construct the selection slices
    sel = [slice(start, end) for start, end in zip(corner, end_points)]
    sel_tuple = tuple(sel)
    print("sel:", sel)
    print("Type of sel:", type(sel))
    img = np.array(whole_images[sel_tuple], dtype=np.float32)
...
def save_segmentation(corner, canvas, seg_result_path):
    ...
    seg_file=os.path.join(seg_result_path, f'seg_{corner[0]}_{corner[1]}_{corner[2]}_{FLAGS.model_name}')
    np.savez_compressed(seg_file, 
                        segmentation=seg, 
    ...
    prob_file=os.path.join(seg_result_path, f'prob_{corner[0]}_{corner[1]}_{corner[2]}_{FLAGS.model_name}')
    
    np.savez_compressed(prob_file, 
                        qprob=prob)
